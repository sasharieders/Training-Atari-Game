...
def train_agent(n_iterations, checkpoint_dir):
    time_step = None
...
def load_and_resume_training(checkpoint_path):
    checkpoint = tf.train.Checkpoint(agent=agent)
    checkpoint.restore(checkpoint_path)
    start_iteration = int(checkpoint_path.split("_")[1].split("-")[0])
    remaining_iterations = 2000 - start_iteration
    trajectories = train_agent(remaining_iterations, checkpoint_dir)

checkpoint_path = os.path.join(checkpoint_dir, "checkpoint_0-1")
load_and_resume_training(checkpoint_path)
